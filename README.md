...
3. Fill in the env vars Render prompts for (marked `sync: false` in `render.yaml`): `MONGODB_URL`, `GROQ_API_KEY`, `GOOGLE_CLIENT_ID`, `SMTP_USER`, `SMTP_PASSWORD`, `FROM_EMAIL`, `FRONTEND_URL` (your Vercel URL — you can circle back and fill this in after deploying the frontend). `JWT_SECRET` is auto-generated by Render; everything else has a sensible default already in the file.
